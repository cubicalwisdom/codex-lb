...
Claude Desktop's host loop exposes `mcp__workspace__web_fetch` as an eager client function. Large pages are downloaded locally and replaced by Claude Code with a saved-file error marker before Codex LB receives the following request. Add route-level RED coverage proving that an authenticated Desktop turn with an explicit public URL replaces this eager alias with the existing hosted `web_search` tool, while private/local, non-Desktop, and deferred ToolSearch cases remain ordinary functions. Deduplicate the alias with official web server tools, map a forced alias choice only when replacement occurred, then mirror and live-retry `https://claude.com/docs` after the app-owned CodexNeo restart.

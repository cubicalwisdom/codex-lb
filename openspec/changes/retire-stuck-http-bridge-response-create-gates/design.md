...
Reconnect rejects a tombstoned generation before account selection or socket open and rechecks the tombstone before installing a provisional replacement. Until installation, the provisional socket and newly acquired lease remain local ownership: cancellation, retirement, or any other exception closes/releases them before re-raising the original failure. A reused session lease is never treated as provisional, and the old session lease remains attached until its release completes so concurrent retirement can still observe and settle it. The timed-out waiter still receives the existing stable `response_create_gate_timeout` error; a later client retry may create a new generation, but the retired generation can never reconnect, replay, resend, or reacquire ownership.

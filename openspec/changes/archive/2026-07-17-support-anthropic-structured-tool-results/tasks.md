...
- [x] 3.4 Retry the existing poisoned Claude conversation, then run fresh WebFetch, WebSearch, ToolSearch, and ordinary tool-call acceptance. The poisoned `/schedule` retry passes the former converter failure. A fresh Desktop task completed ToolSearch plus ordinary Task calls, WebSearch succeeded in Auto mode, and Workspace WebFetch returned HTTP 200 with `Example Domain` in Manual mode; Auto mode's separate `User Deny Rules` classifier blocks that client-side alias before adapter execution.

...
from app.core.clients.proxy import CODEX_RESPONSES_LITE_WS_METADATA_KEY, ProxyResponseError
...
@pytest.mark.asyncio
async def test_http_bridge_completed_custom_tool_call_records_typed_pending_state(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    ...
    first_request = proxy_service._WebSocketRequestState(
        request_id="req-custom-tool-turn-1",
        model="gpt-5.6-sol",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
    ...
    session = _make_bridge_session(
        key_value="bridge-custom-tool-continuation",
        ...
        json.dumps(
            {
                ...
                "item": {
                    ...
                    "call_id": "call_custom_turn_1",
                    "name": "exec",
                    "input": "run the requested command",
                },
            }
        ),
    )
    ...
    assert session.last_completed_response_id == "resp_custom_tool_turn_1"
    assert session.last_pending_function_call_ids == ["call_custom_turn_1"]
    assert session.last_pending_tool_call_output_types == {
        "call_custom_turn_1": "custom_tool_call_output",
    }


@pytest.mark.asyncio
async def test_http_bridge_completion_without_response_id_keeps_pending_calls_bound_to_prior_anchor(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    monkeypatch.setattr(service, "_finalize_websocket_request_state", AsyncMock())
    monkeypatch.setattr(service, "_register_http_bridge_previous_response_id", AsyncMock())
    request_state = proxy_service._WebSocketRequestState(
        request_id="req-unbound-completion",
        model="gpt-5.4",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
    ...
    session = _make_bridge_session(
        key_value="bridge-unbound-completion",
        pending_requests=deque([request_state]),
        queued_request_count=1,
    )
    session.last_completed_response_id = "resp_prior"
    session.last_pending_function_call_ids = ["call_prior"]
    session.last_pending_tool_call_output_types = {
        "call_prior": "custom_tool_call_output",
    }
    ...
    assert session.last_completed_response_id == "resp_prior"
    assert session.last_pending_function_call_ids == ["call_prior"]
    assert session.last_pending_tool_call_output_types == {
        "call_prior": "custom_tool_call_output",
    }
...
@pytest.mark.parametrize(
    ("turn_metadata", "expected_request_kind"),
    [
        ({"request_kind": "normal"}, "normal"),
        ({"request_kind": "prewarm"}, "prewarm"),
        ({"request_kind": "unknown"}, "normal"),
    ],
)
def test_prepare_http_bridge_request_parses_request_kind_metadata(
    ...
    request_state, _text_data = service._prepare_http_bridge_request(
        ...
        api_key=None,
        api_key_reservation=None,
        request_id="req-request-kind",
    )

    assert getattr(request_state, "request_kind", None) == expected_request_kind


@pytest.mark.parametrize(
    ("request_kind", "expect_continuity_replaced"),
    [("prewarm", False), ("normal", True)],
)
@pytest.mark.asyncio
async def test_process_http_bridge_empty_completion_respects_request_kind(
    monkeypatch: pytest.MonkeyPatch,
    request_kind: str,
    expect_continuity_replaced: bool,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    finalize = AsyncMock()
    ...
    request_state = proxy_service._WebSocketRequestState(
        request_id=f"req-empty-{request_kind}",
        model="gpt-5.4",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
    ...
    request_state.request_kind = request_kind
    request_state.pending_function_call_ids = ["call_new"]
    request_state.pending_tool_call_output_types = {
        "call_new": "custom_tool_call_output",
    }
    session = _make_bridge_session(
        key_value=f"bridge-empty-{request_kind}",
        pending_requests=deque([request_state]),
        queued_request_count=1,
    )
    session.last_completed_response_id = "resp_real_anchor"
    session.last_completed_input_count = 2
    session.last_completed_input_prefix_fingerprint = "real-fingerprint"
    session.last_pending_function_call_ids = ["call_real"]
    session.last_pending_tool_call_output_types = {
        "call_real": "function_call_output",
    }
        ...
        json.dumps(
            {
                ...
                "response": {"id": "resp_empty_candidate", "status": "in_progress"},
            }
        ),
    )
    await service._process_http_bridge_upstream_text(
    ...
    if expect_continuity_replaced:
        assert session.last_completed_response_id == "resp_empty_candidate"
        assert session.last_pending_function_call_ids == ["call_new"]
        assert session.last_pending_tool_call_output_types == {
            "call_new": "custom_tool_call_output",
        }
        register_previous_response_id.assert_awaited_once_with(
            session,
            "resp_empty_candidate",
            input_item_count=None,
            input_full_fingerprint=None,
        )
    else:
        assert session.last_completed_response_id == "resp_real_anchor"
        assert session.last_completed_input_count == 2
        assert session.last_completed_input_prefix_fingerprint == "real-fingerprint"
        assert session.last_pending_function_call_ids == ["call_real"]
        assert session.last_pending_tool_call_output_types == {
            "call_real": "function_call_output",
        }
        register_previous_response_id.assert_not_awaited()
    finalize.assert_awaited_once()


@pytest.mark.asyncio
async def test_maybe_prewarm_marks_synthetic_request_and_preserves_real_continuity(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    visible_request = proxy_service._WebSocketRequestState(
        request_id="req-visible-after-prewarm",
        model="gpt-5.4",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
    ...
    session = _make_bridge_session(key_value="bridge-prewarm-continuity")
    ...
    async def send_text(text_data: str) -> None:
        captured["payload"] = json.loads(text_data)
        assert session.pending_requests
        warmup_state = session.pending_requests[-1]
        captured["request_kind"] = getattr(warmup_state, "request_kind", None)
        await service._process_http_bridge_upstream_text(
            session,
            json.dumps(
                {
                    "type": "response.created",
                    "response": {"id": "resp_generated_prewarm", "status": "in_progress"},
                }
            ),
        )
        await service._process_http_bridge_upstream_text(
    ...
    assert captured["payload"]["generate"] is False
    assert captured["request_kind"] == "prewarm"
    assert session.last_completed_response_id == "resp_real_anchor"
    assert session.last_completed_input_count == 2
    assert session.last_completed_input_prefix_fingerprint == "real-fingerprint"
    assert session.last_pending_function_call_ids == ["call_real"]
...
@pytest.mark.asyncio
async def test_stream_via_http_bridge_prepares_injected_outputs_before_usage_budgeting(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    input_items: list[proxy_service.JsonValue] = [
    ...
    payload = proxy_service.ResponsesRequest.model_validate(
        {
            "model": "gpt-5.4",
            "instructions": "Continue the task.",
            "previous_response_id": "resp_interrupted",
            "input": input_items,
        }
    )
    session = _make_bridge_session(key_value="sid-interrupted-prepare")
    session.last_completed_response_id = "resp_interrupted"
    session.last_pending_function_call_ids = ["call_custom_shell"]
    session.last_pending_tool_call_output_types = {
        "call_custom_shell": "custom_tool_call_output",
    }

    prepared_inputs: list[list[proxy_service.JsonValue]] = []
    ...
    def recording_prepare(
        ...
        **kwargs: Any,
        ...
        prepared_inputs.append(cast(list[proxy_service.JsonValue], prepared_payload.input))
        return real_prepare(prepared_payload, prepared_headers, **kwargs)

    captured: dict[str, Any] = {}

    async def capture_session_events(
    ...
    def usage_budget(prepared_payload: proxy_service.ResponsesRequest):
        assert isinstance(prepared_payload.input, list)
        return proxy_service.ApiKeyRequestUsageBudget(input_tokens=len(prepared_payload.input))
    ...
    monkeypatch.setattr(service, "_stream_http_bridge_session_events", capture_session_events)
    monkeypatch.setattr(http_bridge_request_submit_module, "estimate_api_key_request_usage", usage_budget)

    chunks = [
        chunk
        async for chunk in service._stream_via_http_bridge(
            payload,
            headers={"x-codex-session-id": "sid-interrupted-prepare"},
    ...
    synthetic_output: proxy_service.JsonValue = {
    ...
    expected_input = [synthetic_output, *input_items]
    assert chunks == []
    assert prepared_inputs == [input_items, expected_input]
    assert captured["payload"]["input"] == expected_input
    request_state = cast(proxy_service._WebSocketRequestState, captured["request_state"])
    assert request_state.input_item_count == 2
    assert request_state.input_full_fingerprint == proxy_service._fingerprint_input_items(expected_input)
    assert request_state.request_usage_budget == proxy_service.ApiKeyRequestUsageBudget(input_tokens=2)


@pytest.mark.parametrize("trim_path", ["store_context", "previous_response_replay"])
@pytest.mark.asyncio
async def test_stream_via_http_bridge_persists_final_injected_input_metadata_after_trim(
    monkeypatch: pytest.MonkeyPatch,
    trim_path: str,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    ...
    stored_prefix: list[proxy_service.JsonValue] = []
    if trim_path == "store_context":
        stored_prefix = [
            {"role": "user", "content": [{"type": "input_text", "text": "first"}]},
            {"role": "user", "content": [{"type": "input_text", "text": "second"}]},
        ]
        input_items = [*stored_prefix, next_input]
        expected_upstream_input: list[proxy_service.JsonValue] = [next_input]
    else:
        replayed_output: proxy_service.JsonValue = {
            "type": "custom_tool_call_output",
        ...
        expected_upstream_input = [replayed_output, next_input]

    payload = proxy_service.ResponsesRequest.model_validate(
        {
            "model": "gpt-5.4",
            "instructions": "Continue the task.",
            "previous_response_id": "resp_interrupted_trim",
            "input": input_items,
        }
    )
    session = _make_bridge_session(key_value=f"sid-interrupted-{trim_path}")
    session.last_completed_response_id = "resp_interrupted_trim"
    session.last_pending_function_call_ids = ["call_pending"]
    session.last_pending_tool_call_output_types = {
        "call_pending": "custom_tool_call_output",
    }
    if stored_prefix:
        session.last_completed_input_count = len(stored_prefix)
        session.last_completed_input_prefix_fingerprint = proxy_service._fingerprint_input_items(stored_prefix)
    ...
    expected_upstream_input = [synthetic_output, *expected_upstream_input]
    captured: dict[str, Any] = {}
    register_previous_response_id = AsyncMock()

    async def complete_session_events(
        ...
        **_: Any,
        ...
        captured["upstream_input"] = json.loads(text_data)["input"]
        ...
        await service._process_http_bridge_upstream_text(
            ...
            json.dumps(
                {
                    "type": "response.created",
                    "response": {"id": "resp_repaired", "status": "in_progress"},
                }
            ),
        )
        await service._process_http_bridge_upstream_text(
    ...
    monkeypatch.setattr(service, "_resolve_websocket_previous_response_owner", AsyncMock(return_value="acc-bridge"))
    monkeypatch.setattr(service, "_get_or_create_http_bridge_session", AsyncMock(return_value=session))
    monkeypatch.setattr(service, "_stream_http_bridge_session_events", complete_session_events)
    monkeypatch.setattr(service, "_finalize_websocket_request_state", AsyncMock())
    monkeypatch.setattr(service, "_register_http_bridge_previous_response_id", register_previous_response_id)
    ...
    chunks = [
        ...
        async for chunk in service._stream_via_http_bridge(
            payload,
            headers={"x-codex-session-id": f"sid-interrupted-{trim_path}"},
            codex_session_affinity=True,
            propagate_http_errors=False,
            openai_cache_affinity=False,
            api_key=None,
    ...
    expected_fingerprint = proxy_service._fingerprint_input_items(expected_upstream_input)
    assert chunks == []
    assert captured["upstream_input"] == expected_upstream_input
    assert session.last_completed_input_count == len(expected_upstream_input)
    assert session.last_completed_input_prefix_fingerprint == expected_fingerprint
    register_previous_response_id.assert_awaited_once_with(
        session,
        "resp_repaired",
        input_item_count=len(expected_upstream_input),
        input_full_fingerprint=expected_fingerprint,
    )
...
@pytest.mark.asyncio
async def test_reconnect_http_bridge_session_rejects_retired_generation_before_reopen(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    session = _make_bridge_session(key_value="bridge-retired-generation")
    session.closed = True
    if hasattr(session, "retired"):
        session.retired = True
    else:
        monkeypatch.setattr(type(session), "retired", True, raising=False)
    request_state = proxy_service._WebSocketRequestState(
        request_id="req-retired-generation-reconnect",
        model="gpt-5.6-sol",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
    ...
    ensure_fresh = AsyncMock(return_value=session.account)
    ...
    open_upstream = AsyncMock(return_value=replacement_upstream)
    monkeypatch.setattr(proxy_service, "get_settings", lambda: _make_app_settings())
    monkeypatch.setattr(
        proxy_service,
        "get_settings_cache",
    ...
    resurrection_gaps: list[str] = []
    if not rejected:
        resurrection_gaps.append("retired generation reconnect completed")
    if select_account.await_count:
        resurrection_gaps.append("retired generation reacquired account selection")
    if open_upstream.await_count:
        resurrection_gaps.append("retired generation opened a replacement upstream")
    if not session.closed:
        resurrection_gaps.append("retired generation was reopened")
...
@pytest.mark.asyncio
async def test_reconnect_http_bridge_session_transfers_displaced_ownership_before_background_wait(
    monkeypatch: pytest.MonkeyPatch,
    ownership_mode: str,
) -> None:
    ...
    try:
        ...
        with anyio.fail_after(1.0):
            while not reuse_existing_lease and not cancel_during_old_lease_release and not completed_releases:
                await asyncio.sleep(0)

        assert session.account is selected_account
        assert session.upstream is provisional_upstream
        expected_installed_lease = existing_lease if reuse_existing_lease else provisional_lease
        assert session.account_lease is expected_installed_lease
        assert service._background_cleanup_tasks
        provisional_upstream.close.assert_not_awaited()

        allow_old_settlement.set()
        await service._drain_http_bridge_background_cleanup_tasks(reason=f"test-{ownership_mode}")

        cast(AsyncMock, cast(Any, original_upstream).close).assert_awaited_once_with()
        release_attempts = [call.args[0] for call in release_account_lease.await_args_list]
        assert release_attempts == ([] if reuse_existing_lease else [existing_lease])
        assert completed_releases == ([] if reuse_existing_lease else [existing_lease])
        assert provisional_lease not in completed_releases
...
async def test_stream_via_http_bridge_injects_durable_anchor_for_trimmable_full_resend(
    ...
    session.last_completed_response_id = "resp_stale"
    session.last_pending_function_call_ids = ["call_stale"]
    session.last_pending_tool_call_output_types = {
        "call_stale": "custom_tool_call_output",
    }
    ...
    assert [frame["client_metadata"][CODEX_RESPONSES_LITE_WS_METADATA_KEY] for frame in prepared_frames] == [
        "true",
        "true",
        "true",
    ]
    assert session.last_completed_response_id == "resp_latest"
    assert session.last_pending_function_call_ids == []
    assert session.last_pending_tool_call_output_types == {}
...
async def _run_owner_forward_recovery_with_session(
    ...
    *,
    ...
    async def fail_owner_forward(**_: object):
        raise ProxyResponseError(400, proxy_service.openai_error("previous_response_not_found", "missing"))
        yield ""

    async def complete_submit(
        _session: proxy_service._HTTPBridgeSession,
        *,
        request_state: proxy_service._WebSocketRequestState,
        text_data: str,
    ...
    monkeypatch.setattr(service, "_prepare_http_bridge_request", fake_prepare)
    monkeypatch.setattr(service, "_get_or_create_http_bridge_session", get_or_create)
    monkeypatch.setattr(service, "_forward_http_bridge_request_to_owner", fail_owner_forward)
    monkeypatch.setattr(service, "_submit_http_bridge_request", complete_submit)
    monkeypatch.setattr(service, "_detach_http_bridge_request", AsyncMock())
...
@pytest.mark.asyncio
async def test_stream_via_http_bridge_owner_forward_recovery_reinjects_typed_output(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    recovery_session = _make_owner_forward_recovery_session()
    recovery_session.last_completed_response_id = "resp_prev_1"
    recovery_session.last_pending_function_call_ids = ["call_custom_shell"]
    recovery_session.last_pending_tool_call_output_types = {
        "call_custom_shell": "custom_tool_call_output",
    }
...
@pytest.mark.asyncio
async def test_stream_via_http_bridge_local_recovery_keeps_injected_typed_output(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    input_items = [{"role": "user", "content": "continue"}]
    ...
    failing_session.last_pending_function_call_ids = ["call_patch"]
    failing_session.last_pending_tool_call_output_types = {
        "call_patch": "apply_patch_call_output",
    }
    ...
    async def submit_then_fail_once(
        _session: proxy_service._HTTPBridgeSession,
        *,
        request_state: proxy_service._WebSocketRequestState,
        text_data: str,
    ...
    monkeypatch.setattr(service, "_prepare_http_bridge_request", fake_prepare)
    monkeypatch.setattr(service, "_get_or_create_http_bridge_session", get_or_create)
    monkeypatch.setattr(service, "_submit_http_bridge_request", submit_then_fail_once)
    ...
    assert prepared_inputs == [input_items, [synthetic_output, *input_items], [synthetic_output, *input_items]]
...
async def test_process_http_bridge_upstream_text_retries_precreated_usage_limit(
    ...
    assert request_state.upstream_event_seen is True
    assert request_state.event_queue is not None
...
@pytest.mark.asyncio
async def test_response_create_gate_timeout_remains_stable_when_retirement_cleanup_fails(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    session = _make_bridge_session(key_value="bridge-stuck-cleanup-failure")
    await session.response_create_gate.acquire()
    waiter = proxy_service._WebSocketRequestState(
        request_id="req-stuck-cleanup-failure",
        model="gpt-5.6-sol",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
    ...
    monkeypatch.setattr(
        service,
        "_retire_stuck_http_bridge_session_if_eligible",
...
def test_http_bridge_stuck_gate_retirement_threshold_defaults_to_upstream_value() -> None:
    assert Settings().http_responses_session_bridge_stuck_gate_retire_after_seconds == 300.0


@pytest.mark.asyncio
async def test_response_create_gate_timeout_retires_session_with_old_event_free_http_request(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    settings = _make_app_settings(
    ...
    monkeypatch.setattr(proxy_service, "get_settings", lambda: settings)
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    session = _make_bridge_session(key_value="bridge-stuck-gate")
    await session.response_create_gate.acquire()
    event_queue: asyncio.Queue[str | None] = asyncio.Queue()
    api_key = _make_api_key(key_id="key-stuck-gate", assigned_account_ids=[])
    reservation = proxy_service.ApiKeyUsageReservationData(
        reservation_id="reservation-stuck-gate",
        key_id=api_key.id,
        model="gpt-5.6-sol",
    )
    ...
    old_pending.response_create_gate_acquired_at = time.monotonic() - 301.0
    waiter = proxy_service._WebSocketRequestState(
    ...
    heartbeat_task = asyncio.create_task(heartbeat_stop.wait())
    old_pending.api_key_reservation_heartbeat_stop = heartbeat_stop
    old_pending.api_key_reservation_heartbeat_task = heartbeat_task
    async with session.pending_lock:
        session.pending_requests.append(old_pending)
        session.queued_request_count = 1
    receive_started = asyncio.Event()
    close_woke_receive = asyncio.Event()
    replay_receive_started = asyncio.Event()
    hold_replayed_reader = asyncio.Event()
    receive_count = 0
    ...
    async def receive() -> UpstreamWebSocketMessage:
        ...
        if receive_count == 1:
            receive_started.set()
            await close_woke_receive.wait()
            return UpstreamWebSocketMessage(kind="close", close_code=1011)
        replay_receive_started.set()
        await hold_replayed_reader.wait()
        return UpstreamWebSocketMessage(kind="close", close_code=1011)

    async def close_upstream() -> None:
        close_woke_receive.set()
    ...
    session_lease = proxy_service.AccountLease(
        lease_id="session-stuck-gate",
        account_id=session.account.id,
        kind="stream",
        acquired_at=1.0,
    )
    session.account_lease = session_lease
    session.durable_session_id = "durable-stuck-gate"
    session.durable_owner_epoch = 9
    session.downstream_turn_state_aliases.add("http_turn_stuck_gate")
    session.previous_response_ids.add("resp_stuck_gate")
    service._http_bridge_sessions[session.key] = session
    service._http_bridge_turn_state_index[
        proxy_service._http_bridge_turn_state_alias_key("http_turn_stuck_gate", session.key.api_key_id)
    ] = session.key
    service._http_bridge_previous_response_index[
        proxy_service._http_bridge_previous_response_alias_key("resp_stuck_gate", session.key.api_key_id)
    ] = session.key
    ...
    try:
        ...
        with anyio.fail_after(1.0):
            while not reader.done() and not replay_receive_started.is_set():
                await asyncio.sleep(0)

        queued_events: list[str | None] = []
        while not event_queue.empty():
            queued_events.append(event_queue.get_nowait())
        cleanup_gaps: list[str] = []
        if reconnect.await_count:
            cleanup_gaps.append("retired reader attempted reconnect")
        if send_text.await_count:
            cleanup_gaps.append("retired reader resent response.create")
        if not reader.done() or not reader.cancelled():
            cleanup_gaps.append("upstream reader was not cancelled and awaited")
        if session.upstream_reader is not None:
            cleanup_gaps.append("session retained its upstream reader task")
        if session.pending_requests:
            cleanup_gaps.append("pending request was not removed")
        if session.queued_request_count:
            cleanup_gaps.append("queued request count was not cleared")
        if session.response_create_gate.locked():
            cleanup_gaps.append("response-create gate was not released")
        if old_pending.response_create_admission is not None or not admission_release.called:
            cleanup_gaps.append("response-create admission lease was not released")
        if old_pending.account_response_create_lease is not None or not account_response_create_release.await_count:
            cleanup_gaps.append("account response-create lease was not released")
        if not heartbeat_task.done():
            cleanup_gaps.append("reservation heartbeat task was not settled")
        if release_reservation.await_count != 1:
            cleanup_gaps.append("API-key reservation was not released")
        if len(queued_events) != 2 or queued_events[-1:] != [None]:
            cleanup_gaps.append("old event queue was not terminalized")
        elif queued_events[0] is None or '"code":"stream_incomplete"' not in queued_events[0]:
            cleanup_gaps.append("old event queue did not receive stream_incomplete")
        ...
        release_live_session.assert_awaited_once_with(
            session_id="durable-stuck-gate",
            instance_id=settings.http_responses_session_bridge_instance_id,
            owner_epoch=9,
            draining=False,
        )
        release_account_lease.assert_any_await(session_lease)
        assert sum(call.args == (session_lease,) for call in release_account_lease.await_args_list) == 1
        assert session.account_lease is None
        assert cleanup_gaps == [], "; ".join(cleanup_gaps)
    finally:
        for task in (reader, heartbeat_task):
            if not task.done():
                task.cancel()
            try:
                await task
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("recent-precreated", {"age_seconds": 299.0}),
        ("synthetic-prewarm", {"skip_request_log": True}),
        ("non-http", {"transport": "websocket"}),
    ],
    ids=lambda value: value if isinstance(value, str) else None,
)
@pytest.mark.asyncio
async def test_response_create_gate_timeout_preserves_ineligible_pending_work(
    monkeypatch: pytest.MonkeyPatch,
    case: str,
    ...
    monkeypatch.setattr(proxy_service, "get_settings", lambda: settings)
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    session = _make_bridge_session(key_value="bridge-preserve-gate")
    await session.response_create_gate.acquire()
    age_seconds = float(state_updates.get("age_seconds", 301.0))
    ...
    pending.response_create_gate_acquired_at = time.monotonic() - age_seconds
    for name, value in state_updates.items():
        if name == "age_seconds":
            continue
        if name == "upstream_event_seen" and not hasattr(pending, name):
            monkeypatch.setattr(type(pending), name, value, raising=False)
        else:
            setattr(pending, name, value)
    waiter = proxy_service._WebSocketRequestState(
        request_id="req-preserve-waiter",
        model="gpt-5.6-sol",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
...
@pytest.mark.asyncio
async def test_stuck_gate_retirement_uses_gate_age_not_total_request_age(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    session = _make_bridge_session(key_value="bridge-recent-gate")
    gate = session.response_create_gate
    await gate.acquire()
    now = time.monotonic()
    pending = proxy_service._WebSocketRequestState(
        request_id="req-old-with-recent-gate",
        model="gpt-5.6-sol",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
    ...
    pending.response_create_gate_acquired_at = now - 1.0
    session.pending_requests.append(pending)
    session.queued_request_count = 1
    service._http_bridge_sessions[session.key] = session
    close = AsyncMock()
    ...
    monkeypatch.setattr(service, "_close_http_bridge_session_bounded", close_bounded)

    retired = await service._retire_stuck_http_bridge_session_if_eligible(
        session,
...
@pytest.mark.asyncio
async def test_stuck_gate_retirement_caller_cancellation_keeps_detached_cleanup_running(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    session = _make_bridge_session(key_value="bridge-retire-cancel-safe")
    gate = session.response_create_gate
    await gate.acquire()
    pending = proxy_service._WebSocketRequestState(
        request_id="req-retire-cancel-safe",
        model="gpt-5.6-sol",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
    ...
    pending.response_create_gate_acquired_at = time.monotonic() - 600.0
    session.pending_requests.append(pending)
    session.queued_request_count = 1
    session.account_lease = proxy_service.AccountLease(
        lease_id="lease-retire-cancel-safe",
        account_id=session.account.id,
        kind="stream",
        acquired_at=1.0,
    )
    ...
    async def release_account_lease(_lease: proxy_service.AccountLease | None) -> None:
        release_started.set()
    ...
    close = cast(AsyncMock, cast(Any, session.upstream).close)
    ...
    try:
        ...
        assert session.queued_request_count == 0
        assert gate.locked() is False
        assert pending.response_create_gate is None
        assert pending.response_create_gate_acquired is False
    finally:
        allow_release.set()
        ...
        if not reader.done():
            reader.cancel()
        await asyncio.gather(retirement, reader, return_exceptions=True)
        for cleanup_task in list(service._background_cleanup_tasks):
            await asyncio.gather(cleanup_task, return_exceptions=True)


@pytest.mark.parametrize("retry_kind", ["fresh", "visible", "auth", "security"])
@pytest.mark.asyncio
async def test_stuck_gate_retirement_cancels_retry_send_after_terminal_race(
    monkeypatch: pytest.MonkeyPatch,
    retry_kind: str,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    session = _make_bridge_session(key_value=f"bridge-retire-send-{retry_kind}")
    ...
    gate = session.response_create_gate
    await gate.acquire()
    initially_precreated = retry_kind in {"fresh", "security"}
    request_state = proxy_service._WebSocketRequestState(
        request_id=f"req-retire-send-{retry_kind}",
        model="gpt-5.6-sol",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
        ...
        response_id=None if initially_precreated else "resp-reset-by-real-helper",
        response_event_count=0 if initially_precreated else 1,
        response_create_gate_acquired=True,
        response_create_gate=gate,
        awaiting_response_created=initially_precreated,
        event_queue=asyncio.Queue(),
    )
    request_state.response_create_gate_acquired_at = time.monotonic() - 600.0
    ...
    cast(Any, session.upstream).send_text = send_text
    monkeypatch.setattr(service, "_reconnect_http_bridge_session", AsyncMock())
    monkeypatch.setattr(service, "_write_request_log", AsyncMock())

    if retry_kind == "fresh":
        retry = service._retry_http_bridge_request_on_fresh_upstream(
            session,
            request_state=request_state,
            text_data=cast(str, request_state.request_text),
        )
        expected_result: object = False
    elif retry_kind == "visible":
        retry = service._retry_http_bridge_precreated_request(session)
        expected_result = False
    elif retry_kind == "auth":
        retry = service._retry_http_bridge_precreated_auth_request(
            session,
            request_state,
            error_message=None,
        )
        expected_result = "not_replayable"
    else:
        retry = service._retry_http_bridge_security_work_request(session, request_state)
        expected_result = False
    retry_task = asyncio.create_task(retry)

    try:
        await asyncio.wait_for(send_entered.wait(), timeout=1.0)
        if retry_kind in {"visible", "auth"}:
        ...
        assert await asyncio.wait_for(retry_task, timeout=1.0) == expected_result
        assert session.retired is True
        assert session.closed is True
        assert session.key not in service._http_bridge_sessions
        assert list(session.pending_requests) == []
...
@pytest.mark.parametrize("cleanup_phase", ["provisional-close", "provisional-lease-release"])
@pytest.mark.asyncio
async def test_reconnect_http_bridge_session_repeated_cancellation_cannot_abandon_provisional_cleanup(
    monkeypatch: pytest.MonkeyPatch,
    cleanup_phase: str,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    session = _make_bridge_session(key_value=f"bridge-double-cancel-{cleanup_phase}")
    original_account = session.account
    original_upstream = session.upstream
    existing_lease = proxy_service.AccountLease(
        lease_id="existing-double-cancel",
        account_id=original_account.id,
        kind="stream",
        acquired_at=1.0,
    )
    session.account_lease = existing_lease
    selected_account = cast(Any, SimpleNamespace(id="acc-double-cancel", status=AccountStatus.ACTIVE))
    provisional_lease = proxy_service.AccountLease(
        lease_id="provisional-double-cancel",
        account_id=selected_account.id,
        kind="stream",
        acquired_at=2.0,
    )
    ...
    provisional_opened = asyncio.Event()
    cleanup_started = asyncio.Event()
    allow_cleanup = asyncio.Event()
    provisional_close_completed = asyncio.Event()
    provisional_release_completed = asyncio.Event()

    async def close_provisional() -> None:
        if cleanup_phase == "provisional-close":
            cleanup_started.set()
            await allow_cleanup.wait()
        provisional_close_completed.set()

    provisional_upstream = cast(
        UpstreamResponsesWebSocket,
        SimpleNamespace(
            close=AsyncMock(side_effect=close_provisional),
            response_header=lambda _name: None,
        ),
    )

    async def open_provisional(*_args: object, **_kwargs: object) -> UpstreamResponsesWebSocket:
        provisional_opened.set()
        return provisional_upstream

    async def release_account_lease(lease: proxy_service.AccountLease | None) -> None:
        if lease is provisional_lease:
            if cleanup_phase == "provisional-lease-release":
                cleanup_started.set()
                await allow_cleanup.wait()
            provisional_release_completed.set()

    release = AsyncMock(side_effect=release_account_lease)
    monkeypatch.setattr(proxy_service, "get_settings", lambda: _make_app_settings())
    monkeypatch.setattr(
        proxy_service,
        "get_settings_cache",
    ...
    monkeypatch.setattr(service, "_ensure_fresh_with_budget", AsyncMock(return_value=selected_account))
    monkeypatch.setattr(service, "_open_upstream_websocket_with_budget", open_provisional)
    monkeypatch.setattr(service._load_balancer, "release_account_lease", release)
    request_state = proxy_service._WebSocketRequestState(
        request_id=f"req-double-cancel-{cleanup_phase}",
        model="gpt-5.6-sol",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
    ...
    try:
        ...
        await asyncio.wait_for(provisional_opened.wait(), timeout=1.0)
        with anyio.fail_after(1.0):
            while session.lifecycle_lock.statistics().tasks_waiting < 1:
                await asyncio.sleep(0)
        reconnect.cancel()
        ...
        allow_cleanup.set()
        with pytest.raises(asyncio.CancelledError):
            await asyncio.wait_for(asyncio.shield(reconnect), timeout=1.0)

        assert provisional_close_completed.is_set()
        assert provisional_release_completed.is_set()
        assert sum(call.args == (provisional_lease,) for call in release.await_args_list) == 1
        assert all(call.args != (existing_lease,) for call in release.await_args_list)
        cast(AsyncMock, cast(Any, original_upstream).close).assert_not_awaited()
...
@pytest.mark.parametrize("handler_kind", ["mark-permanent-failure", "handle-proxy-error"])
@pytest.mark.asyncio
async def test_reconnect_http_bridge_session_handler_body_cancellation_releases_fresh_lease(
    monkeypatch: pytest.MonkeyPatch,
    handler_kind: str,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    session = _make_bridge_session(key_value=f"bridge-handler-cancel-{handler_kind}")
    original_account = session.account
    original_upstream = session.upstream
    existing_lease = proxy_service.AccountLease(
        lease_id="existing-handler-cancel",
        account_id=original_account.id,
        kind="stream",
        acquired_at=1.0,
    )
    session.account_lease = existing_lease
    selected_account = cast(Any, SimpleNamespace(id="acc-handler-cancel", status=AccountStatus.ACTIVE))
    provisional_lease = proxy_service.AccountLease(
        lease_id="provisional-handler-cancel",
        account_id=selected_account.id,
        kind="stream",
        acquired_at=2.0,
    )
    ...
    async def blocked_handler(*_args: object, **_kwargs: object) -> None:
        handler_started.set()
        await asyncio.Event().wait()

    if handler_kind == "mark-permanent-failure":
        ensure_fresh = AsyncMock(side_effect=RefreshError("invalid_grant", "refresh failed", True))
        open_upstream = AsyncMock()
        monkeypatch.setattr(service._load_balancer, "mark_permanent_failure", blocked_handler)
    else:
        ensure_fresh = AsyncMock(side_effect=[selected_account, selected_account])
        unauthorized = ProxyResponseError(
            401,
            proxy_service.openai_error("invalid_api_key", "upstream rejected credentials"),
        )
        open_upstream = AsyncMock(side_effect=[unauthorized, unauthorized])
        monkeypatch.setattr(service, "_handle_proxy_error", blocked_handler)
    release = AsyncMock()
    monkeypatch.setattr(proxy_service, "get_settings", lambda: _make_app_settings())
    monkeypatch.setattr(
        proxy_service,
    ...
    monkeypatch.setattr(service._load_balancer, "release_account_lease", release)
    request_state = proxy_service._WebSocketRequestState(
        request_id=f"req-handler-cancel-{handler_kind}",
        model="gpt-5.6-sol",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
    ...
    release.assert_awaited_once_with(provisional_lease)
    assert session.account is original_account
    assert session.upstream is original_upstream
    assert session.account_lease is existing_lease


@pytest.mark.parametrize("failure_kind", ["terminal-open", "terminal-refresh"])
@pytest.mark.asyncio
async def test_reconnect_http_bridge_session_terminal_failure_retains_reused_lease(
    monkeypatch: pytest.MonkeyPatch,
    failure_kind: str,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    session = _make_bridge_session(key_value=f"bridge-reused-terminal-{failure_kind}")
    original_account = session.account
    original_upstream = session.upstream
    existing_lease = proxy_service.AccountLease(
        lease_id="existing-reused-terminal",
        account_id=original_account.id,
        kind="stream",
        acquired_at=1.0,
    )
    ...
    if failure_kind == "terminal-open":
        started_at = time.monotonic()
        ensure_fresh = AsyncMock(return_value=original_account)
        open_upstream = AsyncMock(
            side_effect=ProxyResponseError(
                502,
                proxy_service.openai_error("upstream_unavailable", "terminal open failure"),
            )
        )
        expected_exception: type[BaseException] = ProxyResponseError
    else:
        started_at = time.monotonic() - 600.0
        ensure_fresh = AsyncMock(side_effect=RefreshError("refresh_failed", "terminal refresh failure", False))
        open_upstream = AsyncMock()
        expected_exception = RefreshError
        monkeypatch.setattr(http_bridge_mixin_module, "_remaining_budget_seconds", lambda _deadline: 0.0)
    release = AsyncMock()
    monkeypatch.setattr(proxy_service, "get_settings", lambda: _make_app_settings())
    monkeypatch.setattr(
        proxy_service,
    ...
    monkeypatch.setattr(service._load_balancer, "release_account_lease", release)
    request_state = proxy_service._WebSocketRequestState(
        request_id=f"req-reused-terminal-{failure_kind}",
        model="gpt-5.6-sol",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
...
@pytest.mark.asyncio
async def test_reconnect_provisional_cleanup_retries_each_unsettled_resource(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    provisional_lease = proxy_service.AccountLease(
        lease_id="provisional-cleanup-retry",
        account_id="acc-cleanup-retry",
        kind="stream",
        acquired_at=1.0,
    )
    provisional_upstream = cast(
        UpstreamResponsesWebSocket,
        SimpleNamespace(
            close=AsyncMock(side_effect=[RuntimeError("close failed once"), None]),
        ),
    )
    release = AsyncMock(side_effect=[RuntimeError("release failed once"), None])
    monkeypatch.setattr(service._load_balancer, "release_account_lease", release)
    monkeypatch.setattr(http_bridge_mixin_module, "_HTTP_BRIDGE_PROVISIONAL_CLEANUP_RETRY_SECONDS", 0.0)

    await service._await_http_bridge_reconnect_provisional_cleanup(
        provisional_upstream,
        provisional_lease,
        request_id="req-cleanup-retry",
    )

    assert cast(AsyncMock, cast(Any, provisional_upstream).close).await_count == 2
...
@pytest.mark.asyncio
async def test_reconnect_provisional_cleanup_timeout_keeps_ownership_tracked(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    provisional_lease = proxy_service.AccountLease(
        lease_id="provisional-cleanup-background",
        account_id="acc-cleanup-background",
        kind="stream",
        acquired_at=1.0,
    )
    close_started = asyncio.Event()
    ...
    provisional_upstream = cast(
        UpstreamResponsesWebSocket,
        SimpleNamespace(close=AsyncMock(side_effect=close_provisional)),
    )
    release = AsyncMock()
    monkeypatch.setattr(service._load_balancer, "release_account_lease", release)
    monkeypatch.setattr(http_bridge_mixin_module, "_HTTP_BRIDGE_BACKGROUND_CLOSE_TIMEOUT_SECONDS", 0.001)
    ...
    await service._await_http_bridge_reconnect_provisional_cleanup(
        provisional_upstream,
        provisional_lease,
        request_id="req-cleanup-background",
    )
...
@pytest.mark.asyncio
async def test_reconnect_http_bridge_session_serializes_concurrent_resource_handoff(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    session = _make_bridge_session(key_value="bridge-concurrent-reconnect")
    original_upstream = session.upstream
    original_lease = proxy_service.AccountLease(
        lease_id="lease-concurrent-original",
        account_id=session.account.id,
        kind="stream",
        acquired_at=1.0,
    )
    session.account_lease = original_lease
    accounts = {
        "reconnect-a": cast(Any, SimpleNamespace(id="acc-concurrent-a", status=AccountStatus.ACTIVE)),
        "reconnect-b": cast(Any, SimpleNamespace(id="acc-concurrent-b", status=AccountStatus.ACTIVE)),
    }
    leases = {
        name: proxy_service.AccountLease(
            lease_id=f"lease-{name}",
            account_id=account.id,
            kind="stream",
            acquired_at=index + 2.0,
        )
    ...
    upstreams = {
        name: cast(
            ...
            SimpleNamespace(
                close=AsyncMock(),
                response_header=lambda _header_name: None,
            ),
        )
        for name in accounts
    }
    open_started = {name: asyncio.Event() for name in accounts}
    allow_open = {name: asyncio.Event() for name in accounts}
    ...
    async def open_upstream(account: Any, *_args: object, **_kwargs: object) -> UpstreamResponsesWebSocket:
        name = "reconnect-a" if account is accounts["reconnect-a"] else "reconnect-b"
        open_started[name].set()
        await allow_open[name].wait()
        return upstreams[name]

    async def hold_reader(_session: proxy_service._HTTPBridgeSession) -> None:
        task = asyncio.current_task()
        assert task is not None
        reader_tasks.append(cast(asyncio.Task[None], task))
        await asyncio.Event().wait()
    ...
    monkeypatch.setattr(service, "_open_upstream_websocket_with_budget", open_upstream)
    monkeypatch.setattr(service, "_relay_http_bridge_upstream_messages", hold_reader)
    monkeypatch.setattr(service._load_balancer, "release_account_lease", release)
    request_states = {
        name: proxy_service._WebSocketRequestState(
    ...
    reconnect_a = asyncio.create_task(
        service._reconnect_http_bridge_session(
            session,
            request_state=request_states["reconnect-a"],
            restart_reader=True,
        ),
        name="reconnect-a",
    )
    reconnect_b: asyncio.Task[None] | None = None

    try:
        await asyncio.wait_for(open_started["reconnect-a"].wait(), timeout=1.0)
        reconnect_b = asyncio.create_task(
            service._reconnect_http_bridge_session(
                session,
                request_state=request_states["reconnect-b"],
                restart_reader=True,
            ),
            name="reconnect-b",
        )
        with pytest.raises(TimeoutError):
            await asyncio.wait_for(open_started["reconnect-b"].wait(), timeout=0.05)

        allow_open["reconnect-a"].set()
        await asyncio.wait_for(reconnect_a, timeout=1.0)
        await asyncio.wait_for(open_started["reconnect-b"].wait(), timeout=1.0)
        allow_open["reconnect-b"].set()
        await asyncio.wait_for(reconnect_b, timeout=1.0)
        with anyio.fail_after(1.0):
            while len(reader_tasks) < 2:
                await asyncio.sleep(0)
        await service._drain_http_bridge_background_cleanup_tasks(reason="test-concurrent-reconnect")

        assert session.account is accounts["reconnect-b"]
        assert session.upstream is upstreams["reconnect-b"]
        assert session.account_lease is leases["reconnect-b"]
        assert cast(AsyncMock, cast(Any, original_upstream).close).await_count == 1
        assert cast(AsyncMock, cast(Any, upstreams["reconnect-a"]).close).await_count == 1
        assert cast(AsyncMock, cast(Any, upstreams["reconnect-b"]).close).await_count == 0
        assert release.await_args_list == [
            ((original_lease,), {}),
            ((leases["reconnect-a"],), {}),
        ]
        assert reader_tasks[0].cancelled()
        assert reader_tasks[1] is session.upstream_reader
        assert reader_tasks[1].done() is False
    finally:
        allow_open["reconnect-a"].set()
        allow_open["reconnect-b"].set()
        tasks = [reconnect_a]
        if reconnect_b is not None:
            tasks.append(reconnect_b)
...
@pytest.mark.asyncio
async def test_reconnect_http_bridge_session_retries_displaced_resource_settlement(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    session = _make_bridge_session(key_value="bridge-displaced-retry")
    old_lease = proxy_service.AccountLease(
        lease_id="lease-displaced-old",
        account_id=session.account.id,
        kind="stream",
        acquired_at=1.0,
    )
    session.account_lease = old_lease
    old_close_attempts = 0
    retry_started = asyncio.Event()
    allow_retry_success = asyncio.Event()

    async def close_old_upstream() -> None:
        nonlocal old_close_attempts
        old_close_attempts += 1
        if old_close_attempts == 1:
            raise RuntimeError("old upstream close failed once")
        retry_started.set()
        await allow_retry_success.wait()

    old_upstream = cast(
        UpstreamResponsesWebSocket,
        SimpleNamespace(close=AsyncMock(side_effect=close_old_upstream)),
    )
    session.upstream = old_upstream
    new_account = cast(Any, SimpleNamespace(id="acc-displaced-new", status=AccountStatus.ACTIVE))
    new_lease = proxy_service.AccountLease(
        lease_id="lease-displaced-new",
    ...
    monkeypatch.setattr(http_bridge_mixin_module, "_HTTP_BRIDGE_PROVISIONAL_CLEANUP_RETRY_SECONDS", 0.0)
    request_state = proxy_service._WebSocketRequestState(
        request_id="req-displaced-retry",
        model="gpt-5.6-sol",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
    ...
    try:
        await asyncio.wait_for(reconnect, timeout=1.0)
        await asyncio.wait_for(retry_started.wait(), timeout=1.0)
        with anyio.fail_after(1.0):
            while release.await_count < 1:
                await asyncio.sleep(0)
        ...
        assert any(
            task.get_name().startswith("http-bridge-reconnect-displaced-")
            for task in service._background_cleanup_tasks
        )
        release.assert_awaited_once_with(old_lease)
        cast(AsyncMock, cast(Any, new_upstream).close).assert_not_awaited()

        allow_retry_success.set()
        await service._drain_http_bridge_background_cleanup_tasks(reason="test-displaced-retry")
        assert old_close_attempts == 2
        assert service._background_cleanup_tasks == set()
    finally:
        allow_retry_success.set()
...
@pytest.mark.asyncio
async def test_reconnect_losing_to_retirement_settles_each_resource_once(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    session = _make_bridge_session(key_value="bridge-reconnect-retirement-race")
    gate = session.response_create_gate
    await gate.acquire()
    pending = proxy_service._WebSocketRequestState(
        request_id="req-reconnect-retirement-race",
        model="gpt-5.6-sol",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
    ...
    session.upstream = old_upstream
    service._http_bridge_sessions[session.key] = session
    new_account = cast(Any, SimpleNamespace(id="acc-race-new", status=AccountStatus.ACTIVE))
    new_lease = proxy_service.AccountLease(
        lease_id="lease-race-new",
        account_id=new_account.id,
        kind="stream",
        acquired_at=2.0,
    )
    ...
    async def open_upstream(*_args: object, **_kwargs: object) -> UpstreamResponsesWebSocket:
        open_started.set()
        return new_upstream

    release_attempts: list[proxy_service.AccountLease | None] = []
    first_old_release_started = asyncio.Event()
    allow_first_old_release = asyncio.Event()
    old_release_count = 0

    async def release_account_lease(lease: proxy_service.AccountLease | None) -> None:
        nonlocal old_release_count
        release_attempts.append(lease)
        if lease is old_lease:
            old_release_count += 1
            if old_release_count == 1:
                first_old_release_started.set()
                await allow_first_old_release.wait()
    ...
    lifecycle_lock_held = False
    retirement: asyncio.Task[bool] | None = None
    reconnect: asyncio.Task[None] | None = None
    ...
    try:
        ...
        with anyio.fail_after(1.0):
            while (
                not first_old_release_started.is_set()
                and session.lifecycle_lock.statistics().tasks_waiting < 2
            ):
                await asyncio.sleep(0)
        ...
        lifecycle_lock_held = False
        with anyio.fail_after(1.0):
            while old_release_count < 2 and not reconnect.done():
                await asyncio.sleep(0)
        allow_first_old_release.set()

        assert await asyncio.wait_for(retirement, timeout=1.0) is True
        with pytest.raises(ProxyResponseError):
            await asyncio.wait_for(reconnect, timeout=1.0)
        await service._drain_http_bridge_background_cleanup_tasks(reason="test-retirement-race")

        assert cast(AsyncMock, cast(Any, old_upstream).close).await_count == 1
        assert cast(AsyncMock, cast(Any, new_upstream).close).await_count == 1
        assert len(release_attempts) == 2
        assert sum(lease is old_lease for lease in release_attempts) == 1
        assert sum(lease is new_lease for lease in release_attempts) == 1
        assert None not in release_attempts
        assert session.retired is True
        ...
        if lifecycle_lock_held:
            session.lifecycle_lock.release()
        tasks = [task for task in (retirement, reconnect) if task is not None]
        for task in tasks:
            if not task.done():
                task.cancel()
        await asyncio.gather(*tasks, return_exceptions=True)
